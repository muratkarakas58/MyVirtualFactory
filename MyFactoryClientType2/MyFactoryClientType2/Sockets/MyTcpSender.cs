...
namespace MyFactoryClientType2.Sockets
{
    public class MyTcpSender
    {
        public static Result<string> Connect(String server, String message, int portAdress)
        {
            ...
            TextBox textBoxSender;

            if (formActive.Name== "FormLogin")
            {
                 textBoxSender = Application.OpenForms["FormLogin"].Controls["textBoxSender"] as TextBox;
            }
            else
            {
                textBoxSender = Application.OpenForms["FormMyFactoryClientType2"].Controls["textBoxSender"] as TextBox;
            }
            ...
            try
            {
                textBoxSender.AppendText($"Connect Start:{Environment.NewLine}");
